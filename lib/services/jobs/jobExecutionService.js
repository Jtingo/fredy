...
/** Below this interval, scheduled runs are jittered to look less machine-regular. */
export const JITTER_THRESHOLD_MS = 5 * 60 * 1000;
/** Maximum random delay applied to a scheduled run when jitter is active. */
export const MAX_JITTER_MS = 30_000;

/**
 * Computes the random delay (jitter) applied to a scheduled run.
 * Intervals below {@link JITTER_THRESHOLD_MS} (5 minutes) are aggressive enough to look
 * bot-like, so each run is offset by a uniform random delay of up to
 * {@link MAX_JITTER_MS} (30 seconds). Longer intervals run without jitter.
 *
 * @param {number} intervalMs - Scheduler interval in milliseconds.
 * @param {() => number} [random=Math.random] - Random source, injectable for tests.
 * @returns {number} Delay in milliseconds (0 when no jitter applies).
 */
export function getScheduleJitterMs(intervalMs, random = Math.random) {
  if (!Number.isFinite(intervalMs) || intervalMs >= JITTER_THRESHOLD_MS) {
    return 0;
  }
  return Math.round(random() * MAX_JITTER_MS);
}

/** Absolute timestamp (ms epoch) of the next scheduled run, jitter already included. */
let nextScheduledRunAt = null;

/**
 * Returns the exact timestamp of the next scheduled run, with the random
 * jitter already applied — the dashboard can display it as-is. `null` when
 * the periodic scheduler is not running in this process.
 *
 * @returns {number|null} ms epoch of the next run, or null
 */
...
export function initJobExecutionService({ providers, settings, intervalMs }) {
  ...
  // Start scheduler and initial run. A self-rescheduling timeout (instead of
  // setInterval) draws the jitter for the next run up front, so the exact next
  // run time is known in advance and can be shown in the dashboard.
  if (Number.isFinite(intervalMs) && intervalMs > 0) {
    const scheduleNextRun = () => {
      const jitterMs = getScheduleJitterMs(intervalMs);
      if (jitterMs > 0) {
        logger.debug(`Next scheduled run delayed by ${jitterMs}ms jitter (interval below 5 minutes).`);
      }
      nextScheduledRunAt = Date.now() + intervalMs + jitterMs;
      setTimeout(() => {
        scheduleNextRun();
        runAll(true);
      }, intervalMs + jitterMs);
