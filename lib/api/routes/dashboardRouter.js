...
import { getNextScheduledRunAt } from '../../services/jobs/jobExecutionService.js';
...
export default async function dashboardPlugin(fastify) {
  fastify.get('/', async (request) => {
    ...
    // The scheduler pre-draws the jitter for its next run, so this timestamp
    // is exact. Falls back to lastRun + interval when the scheduler is not
    // running in this process (e.g. interval <= 0).
    const scheduledNextRun = getNextScheduledRunAt();

    return {
      general: {
        interval: settings.interval,
        lastRun,
        nextRun: scheduledNextRun ?? (lastRun == null ? 0 : lastRun + settings.interval * 60000),
      },
      kpis: {
        totalJobs,
        totalListings,
