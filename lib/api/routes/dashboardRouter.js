...
import { JITTER_THRESHOLD_MS, MAX_JITTER_MS } from '../../services/jobs/jobExecutionService.js';
...
export default async function dashboardPlugin(fastify) {
  fastify.get('/', async (request) => {
    ...
    // Intervals below the jitter threshold delay each run by a random 0..MAX_JITTER_MS,
    // so the UI can render the next run as a window instead of an exact timestamp.
    const jitterMaxSeconds = settings.interval * 60000 < JITTER_THRESHOLD_MS ? MAX_JITTER_MS / 1000 : 0;
    ...
    return {
      general: {
        ...
        nextRunJitterMaxSeconds: jitterMaxSeconds,
      },
      kpis: {
        totalJobs,
        totalListings,
