...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    if (typeof appSettings.interval !== 'undefined') {
      const interval = Number(appSettings.interval);
      if (!Number.isInteger(interval) || interval < 1 || interval > 1440) {
        return reply.code(400).send({ error: 'Interval must be an integer between 1 and 1440 minutes.' });
      }
    }
