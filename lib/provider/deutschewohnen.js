...
const API_PATH = '/api/deuwo-real-estate/list';
 ...
 * search URL (e.g. /mieten/mietangebote?rentType=miete&city=Berlin...); its
...
async function getListings(url) {
  ...
  if (!response.ok) {
    logger.error(`Error fetching deutsche-wohnen listings (HTTP ${response.status}) from ${apiUrl}`);
    return [];
  }
...
function normalize(o) {
  ...
  return {
    ...
    rooms,
    address,
    // The API delivers exact coordinates; pass them through so the spatial
    // (polygon) filter works without relying on geocoding the address.
    latitude: typeof o.lat === 'number' ? o.lat : null,
    longitude: typeof o.lng === 'number' ? o.lng : null,
